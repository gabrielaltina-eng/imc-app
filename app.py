...
def conectar():
    return sqlite3.connect("imc_plus_v2.db", check_same_thread=False)
    ...
    if st.sidebar.button("Salvar Perfil", type="primary"):
        if novo_nome.strip():
            try:
                ...
                conn.commit()
                conn.close()
                st.sidebar.success(f"Perfil '{novo_nome}' cadastrado!")
                st.rerun()
            except Exception:
                st.sidebar.error("Atenção: Nome de perfil já existente.")
        else:
